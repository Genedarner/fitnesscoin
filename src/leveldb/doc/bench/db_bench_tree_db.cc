...
class Benchmark {
  ...
  : db_(NULL),
    ...
    if (!FLAGS_use_existing_db) {
      for (int i = 0; i < files.size(); i++) {
        if (Slice(files[i]).starts_with("dbbench_GYMDB")) {
 ...
 private:
    ...
    snprintf(file_name, sizeof(file_name),
             "%s/dbbench_GYMDB-%d.kct",
             test_dir.c_str(),
             db_num_);

    // Create tuning options and open the database
    int open_options = kyotocabinet::GYMDB::OWRITER |
                       kyotocabinet::GYMDB::OCREATE;
    ...
    if (sync) {
      open_options |= kyotocabinet::GYMDB::OAUTOSYNC;
    }
